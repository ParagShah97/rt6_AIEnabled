...
diag "Testing people inline edit";
{
    ...
    sleep 1.5;
    my $dom = $s->dom;
    is( $dom->at('div.owner div.col div.rt-value .current-value span.user a:nth-of-type(2)')->text, $root->Format, 'Got updated owner' );
    is( $dom->at('div.requestors div.col div.rt-value .current-value span.user a:nth-of-type(2)')->text,
        '<bob@example.com>', 'Got updated requestor' );
    is( $dom->at('div.cc div.col div.rt-value .current-value span.user a:nth-of-type(2)')->text, '<alice@example.com>', 'Got updated cc' );
